...
25. **Header is z-index 10000.** Fullscreen modals that MUST cover the nav use 10001+. Regular page content stays below 10000.
...
- Nav overlap: adjusting z-index from 200→400 (still below header's 10000!) instead of making modal z-index > 10000
...
2. Modal overlay → `z-index: 10001` (ABOVE header's 10000), zoom overlay → `z-index: 10003`
...
- **RULE: Modals that must cover the full viewport need z-index > 10000** (header is 10000). Not 400, not 9999 — it must be HIGHER than the header.
...
3. Modal overlay z-index: 10001 (above header's 10000)
...
29. **Modals need z-index > 10000.** Header is 10000. Full-viewport modals must be higher.
