...
(() => {
  ...
  // ── Popup Mode ─────────────────────────────────────────────
  // Opens as a normal popup. Tab promotion only happens when batch
  // upload starts (to prevent the popup from closing during uploads).
  const isTabMode = window.location.search.includes('mode=tab');
  if (isTabMode) {
