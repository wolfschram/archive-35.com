 ...
 * from Pinterest's redirect and displays it for manual entry into Agent Settings.
 *
 * Route: https://archive-35.com/api/pinterest-callback?code=xxx&state=yyy
 *
 * This page does NOT auto-submit the code — Wolf copies it into the Agent
 * Settings tab, which calls POST /pinterest/oauth/callback on the local Agent API.
 */
...
export async function onRequestGet(context) {
  ...
  // If Pinterest returned an error
  if (error) {
  ...
  if (!code) {
    return new Response(renderPage({
      ...
      message: 'No authorization code received from Pinterest. Please try the OAuth flow again from Agent Settings.',
      code: '',
      state: '',
      isError: true,
    }), {
  ...
  return new Response(renderPage({
    title: 'Pinterest Authorization Successful',
    message: 'Copy the authorization code below and paste it into Agent Settings → Pinterest → "Enter Auth Code".',
    code,
    state,
    isError: false,
  }), {
...
<body>
  <div class="card">
    ...
    <div class="subtitle">Pinterest Integration</div>
